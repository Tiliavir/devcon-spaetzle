...
#   curl -fsSL https://raw.githubusercontent.com/tiliavir/devcon-spaetzle/main/scripts/install-helvetia.sh | bash -s -- --install-dir /custom/path
...
WRAPPER_IMAGE="devcon-spaetzle-helvetia:latest"

usage() {
    cat <<EOF
Usage: $0 [OPTIONS]

Build a local wrapper image for devcon-spaetzle with company certificates.
...
EXAMPLES:
    ...
    # Custom image name
    curl -fsSL https://raw.githubusercontent.com/tiliavir/devcon-spaetzle/main/scripts/install-helvetia.sh | bash -s -- --image my-company/opencode:latest
EOF
}
...
IMAGE="${OPENCODE_IMAGE:-devcon-spaetzle-helvetia:latest}"
...
if [[ "${1:-}" == "--version" ]] || [[ "${1:-}" == "-v" ]]; then
    echo "spaetzle wrapper for devcon-spaetzle (helvetia)"
    echo "Image: ${IMAGE}"
